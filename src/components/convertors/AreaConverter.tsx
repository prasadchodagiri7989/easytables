...
export const AreaConverter = () => {
  const [category, setCategory] = useState(unitCategories[3].value); // Area
  ...
  useEffect(() => {
    ...
    const defaultFromUnit =
      fromParam && units.some((u) => u.value === fromParam)
        ? fromParam
        : units[0]?.value || "";
    const defaultToUnit =
      toParam && units.some((u) => u.value === toParam)
        ? toParam
        : units[1]?.value || "";

    setFromUnit(defaultFromUnit);
    setToUnit(defaultToUnit);
    setDisableSelects(!!(fromParam && toParam));
    setResult(null);
    ...
    <>
      <Breadcrumb className="mb-4">
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbPage>Area Converter</BreadcrumbPage>
        </BreadcrumbList>
      </Breadcrumb>
