...
export const AngleConverter = () => {
  ...
  useEffect(() => {
    ...
    const isValid = (unit: string | null) => unit && units.some(u => u.value === unit);

    const validFrom = isValid(from);
    const validTo = isValid(to);

    const defaultFrom = validFrom ? from! : units[0]?.value || "";
    const defaultTo = validTo ? to! : units[1]?.value || "";
  ...
  const handleSwapUnits = () => {
    ...
    setToUnit(fromUnit);
    setResult(null);
    setDisableSelects(false); // re-enable selects after swap
  };
    ...
    <>
      <Breadcrumb className="mb-4">
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbPage>Angle Converter</BreadcrumbPage>
        </BreadcrumbList>
      </Breadcrumb>
        ...
        <GuidanceSection title={`How to Convert ${fromUnitName} to ${toUnitName}`}>
          ...
          {fromUnit && toUnit && (
            <div className="bg-background p-2 rounded my-2">
              <p>
                10 {fromUnitName} ={" "}
                {convert(10, fromUnit, toUnit).toLocaleString("en-US", {
                ...
                {[1, 5, 10, 20, 50, 100].map((val) => (
                  <tr key={val}>
                    ...
                    <td className="px-3 py-1">
                      {fromUnit && toUnit
                        ? convert(val, fromUnit, toUnit).toLocaleString("en-US", {
                            maximumFractionDigits: 4,
                            minimumFractionDigits: 0,
                          })
                        : ""}
