...
export const JpgToPng = () => {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      jpgFiles.forEach(file => convertToPng(file));
  ...
  const handleDrop = (event: React.DragEvent<HTMLDivElement>) => {
    ...
    if (files.length > 0) {
      Array.from(files).forEach(file => {
        const jpgFile = file as File;
        if (jpgFile.type === "image/jpeg") {
          convertToPng(jpgFile);
        }
      });
    }
  };
  ...
  const downloadImage = (converted: string, name: string) => {
    ...
    link.href = converted;
    link.download = name.replace(/\.jpg$/i, ".png");
      ...
      <div className="max-w-4xl mx-auto">
        <Card className="shadow-md">
          <CardHeader className="bg-primary/5">
            ...
            <CardDescription>Convert your JPG images to PNG format</CardDescription>
          </CardHeader>
          ...
          <CardContent className="pt-6 pb-4 space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Tip: All JPG files will be converted to PNG format.
            </p>
          </CardContent>
        </Card>
        ...
        <GuidanceSection title="How to Use the JPG to PNG Converter">
          <div className="space-y-4">
            <p>Upload one or more .jpg images and they will be automatically converted to .png format. You can preview and download the converted files instantly.</p>
